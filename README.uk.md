...
LiteLLM надає захищені іменовані маршрути:

| Маршрут                    | Провайдер                          | Комбо OmniRoute |
|----------------------------|------------------------------------|-----------------|
| `cloud-sanitized-auto`     | auto (агентний пул, tool-calls)    | `cloud-auto`    |
| `cloud-sanitized-chat`     | auto (чат-пул, широкий free)       | `cloud-chat`    |
...
Є два незалежні пули з власними межами фолбеку:

- `cloud-auto` (агент) обслуговує `cloud-sanitized-auto`. До нього входять лише
  моделі, що проходять стримінговий **tool-call** probe — Hermes потребує
  викликів інструментів і великого контексту. Кандидати: Mistral і Gemini
  (плюс Cerebras після активації білінгу).
- `cloud-chat` (чат) обслуговує `cloud-sanitized-chat`. Він приймає будь-яку
  модель, що стримить звичайну відповідь — виклики інструментів НЕ потрібні.
  Тому тут ширший набір free-моделей: Groq (короткі чат-контексти — ок),
  легші тири Gemini/Mistral тощо.

Оскільки пули розділені, вичерпання агентних квот на tool-моделі не ламає
Open WebUI: чат-пул фолбечиться на власний набір здорових free-моделей.
`refresh_omniroute_combo.py` перевіряє кожен пул за власними критеріями й
оновлює два комбо незалежно. Groq не входить до `cloud-auto` (його поточні
TPM-ліміти відхиляють звичайні контексти Hermes) — для Groq використовуйте
окремий маршрут `cloud-sanitized-groq`. Ненадійні безкоштовні моделі OpenRouter
теж перевіряються, але зазвичай відсіюються через зависання SSE, 401 на
білінгу або rate limit. Провайдерні комбо `cloud-mistral`/`cloud-gemini`/
`cloud-groq` закріплюють один провайдер, зберігаючи той самий захист Presidio.
Після зміни складу комбо повторно запустіть `./provision_omniroute.sh`.

Модель за замовчуванням для `update_models_and_run.sh` задається через
`DEFAULT_MODEL` у `.env` (за замовчуванням `cloud-sanitized-auto`); її можна
переналаштувати флагом `-m <маршрут>` у `hermes-chat`. Маршрути `auto` та
`chat` існують завжди; провайдерні маршрути з'являються в LiteLLM, лише якщо в
`.env` задано відповідний ключ (список маршрутів збирається в `docker-compose.yml`
із наявності `{PROVIDER}_API_KEY`; самі ключі не потрапляють у контейнер
litellm). В Open WebUI маршрут обирається у випадальному списку `/v1/models` —
для звичайної бесіди обирайте `cloud-sanitized-chat`, щоб вичерпання квот
агентом не впливало на чат.

Увага: провайдерний маршрут не перемикається на інших провайдерів — коли квота
безкоштовного тарифу обраного провайдера вичерпана, запит повертається з помилкою
про обмеження (LiteLLM повторює той самий маршрут). Для максимальної стійкості
використовуйте `cloud-sanitized-auto` (агент) і `cloud-sanitized-chat` (чат):
кожен фолбечиться у межах власного пулу.

Скрипт `refresh_omniroute_combo.py` перевіряє цілі окремо для кожного пулу —
агентний пул стримінговим tool-call запитом (вимагає tool-call і фінальну
SSE-подію), чат-пул звичайним потоковим дописом (вимагає контент асистента;
tool-calls не потрібні). Кожне комбо оновлюється лише моделями, що пройшли
критерії свого пулу. Результати кешуються на п'ять хвилин після успіху і на
