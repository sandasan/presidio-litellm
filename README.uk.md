...
Комбо `cloud-auto` використовує безкоштовні моделі Mistral, Gemini та
OpenRouter. Groq підключений для керування й майбутніх маршрутів, але навмисно
виключений із цього комбо: його поточні TPM-ліміти можуть відхилити великі
контексти Hermes ще до початку SSE-потоку. Скрипт провіжинінгу також виключає
моделі OpenRouter, яким потрібен agentic harness. Після зміни складу комбо
повторно запустіть `./provision_omniroute.sh`.
