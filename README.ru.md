...
|-----------------------|-------------------------------------------------------------------------------|
| `cloud-sanitized-auto`| Агентный пул: LiteLLM → OmniRoute, тот выбирает целевую модель сам и фолбэчится между квотами (только модели с tool-calls) |
| `cloud-sanitized-chat`| Чат-пул для Open WebUI: широкий набор free-моделей, включая без tool-calls   |
| `cloud-sanitized-mistral` | Только Mistral (комбо `cloud-mistral`)                                      |
| `cloud-sanitized-gemini`  | Только Gemini (комбо `cloud-gemini`)                                       |
| `cloud-sanitized-groq`    | Только Groq (комбо `cloud-groq`)                                            |

Именованные провайдерные маршруты присутствуют в LiteLLM, только если задан
соответствующий ключ в `.env` (`MISTRAL_API_KEY`/`GEMINI_API_KEY`/`GROQ_API_KEY`).
Маршруты `auto` и `chat` есть всегда. Модель по умолчанию для запуска Hermes-чата
задаётся `DEFAULT_MODEL` в `.env` (по умолчанию `cloud-sanitized-auto`), её можно
переопределить при запуске hermes-chat флагом `-m <маршрут>`.
...
который пишет в `litellm-proxy/config.yaml` статичные маршруты
`cloud-sanitized-auto`, `cloud-sanitized-chat` и `cloud-sanitized-<provider>` →
OmniRoute (комбо `cloud-auto` / `cloud-chat` / `cloud-<provider>`). Модели
выбирает OmniRoute:
...
2. В комбо входят только бесплатные модели; OmniRoute реалтайм-скорит их
   ...
   Приватность: вес выше у no-training провайдеров — Mistral (политика
   no-training), затем Gemini и Groq (не тренируются на API-трафике по
   умолчанию).
...
По умолчанию весь стек работает на маршруте `cloud-sanitized-auto`: OmniRoute сам
выбирает провайдера и модель по здоровью/квоте/цене. Если хочется жёстко
зафиксировать провайдера — LiteLLM генерирует отдельные защищённые маршруты
(список `/v1/models`):

| Маршрут                  | Провайдер | Комбо в OmniRoute |
|--------------------------|-----------|-------------------|
| `cloud-sanitized-auto`   | auto (все) | `cloud-auto`     |
| `cloud-sanitized-chat`   | auto (чат-пул) | `cloud-chat` |
...
1. **`.env`** — `DEFAULT_MODEL=cloud-sanitized-auto` задаёт модель, которой
   `update_models_and_run.sh` запускает Hermes-чат (можно переопределить флагом
   `-m <маршрут>` у `hermes-chat`). В Open WebUI модель выбирается вручную в
   выпадашке (там видны все маршруты из `/v1/models`).
2. Чат-пул `cloud-sanitized-chat` существует всегда и не зависит от агентного:
   он включает модели без tool-calls и живёт в отдельном комбо `cloud-chat`.
   Для «чайной» беседы в Open WebUI выбирайте именно его — даже если агент
   исчерпал квоты инструментальных моделей, чат продолжит работать.
3. Именованные провайдерные маршруты появляются в LiteLLM, только если задан
   ключ провайдера в `.env` (список маршрутов собирается в docker-compose из
   наличия `{PROVIDER}_API_KEY`; в контейнер litellm сами ключи не попадают).
4. Состав комбо `cloud-<provider>` (какие модели провайдера доступны) задаёт
   `provision_omniroute.sh`; актуальный состав `cloud-auto`/`cloud-chat`
   поддерживает `refresh_omniroute_combo.py` по результатам probe (см. выше).

Внимание: конкретный провайдерный маршрут **не фолбэчится** между провайдерами —
если у выбранного провайдера квота бесплатного тира исчерпана, ответ будет с
ошибкой лимита (LiteLLM доретрает по этому же маршруту). Для максимума
устойчивости используйте `cloud-sanitized-auto` (агент) и `cloud-sanitized-chat`
(чат) — они фолбэчатся внутри своего пула.
...
- `provision_omniroute.sh` создаёт/обновляет комбо `cloud-auto` с весами
  приоритета: `mistral` 5, `gemini` 4, `openrouter` 1. Groq в этом комбо не
  участвует (его текущие TPM-лимиты могут отклонять большие контексты Hermes
  ещё до начала SSE-потока) — для Groq есть отдельный именованный маршрут
  `cloud-sanitized-groq`. Порядок применяется идемпотентно при каждом запуске
  (в т.ч. по весам — через PUT к management API), так что уже существующее
  комбо тоже получит новые веса.
- Провайдерные комбо `cloud-mistral` / `cloud-gemini` / `cloud-groq` позволяют
  жёстко выбрать конкретного провайдера через маршруты LiteLLM
  `cloud-sanitized-<provider>` (анонимизация Presidio та же — гардрейл
  применяется ко всем маршрутам одинаково).
...
- В выпадашке модели выберите маршрут — модели подтягиваются из `/v1/models`
  LiteLLM:
  - **`cloud-sanitized-auto`** — OmniRoute сам выберет провайдера/модель
    (рекомендуется по умолчанию);
  - **`cloud-sanitized-mistral`** / **`cloud-sanitized-gemini`** /
    **`cloud-sanitized-groq`** — жёстко конкретный провайдер (доступен только
    при заданном ключе в `.env`).
...
это просто общение с моделями маршрутов `cloud-sanitized-*` через наш стек.
...
- `ensure-stack.sh` — идемпотентная проверка и запуск стека без лишнего `compose up`
...
Перед запуском ACP и задачи `Hermes Stack` выполняется `ensure-stack.sh`.
Если все контейнеры уже работают, команда запуска Docker вообще не вызывается,
поэтому переключение вкладок расширений не перезапускает Open WebUI или Hermes.
Параллельные старты сериализуются через блокировку.

Не запускайте одновременно отдельный ACP task: он создаёт второе ACP-соединение.
Для обновления образов используйте `update_models_and_run.sh` отдельно: это
намеренная операция с `--force-recreate`, которая может прервать активные сессии.
