...
1. Провижининг `provision_omniroute.sh` подключает в OmniRoute провайдеров
  **OpenRouter**, **Gemini**, **Groq**, **Mistral** из `.env-ключей` (или OmniRoute
  ...
  `cloud-auto` из моделей Mistral и Gemini. Cerebras подключён как
  необязательная цель и добавляется только после активации биллинга и
  успешного tool-call probe. Groq подключён для диагностики и будущих
  маршрутов с коротким контекстом, но исключён: его TPM-лимиты отклоняют
  обычные контексты Hermes. Ненадёжные
  бесплатные модели OpenRouter исключены из комбо из-за зависаний SSE и
  отсутствия поддержки tool-calls.
...
- `provision_omniroute.sh`: идемпотентно логинится в management API, подключает
  ...
  с весами приоритета (mistral 5, gemini 4; см. «Провайдерская приватность»).
  Groq остаётся подключённым для управления и будущих
  маршрутов, но его нестабильные free-модели исключены из этого комбо. Если
  комбо уже существует — обновляет веса через PUT. Повторный запуск безопасен.

`refresh_omniroute_combo.py` проверяет каждую цель потоковым запросом с
tool-call и обновляет комбо только моделями, которые вернули tool-call и
финальное SSE-событие. Успешная проверка кэшируется на пять минут, ошибка — на
пятнадцать. Если все цели временно недоступны, предыдущее комбо сохраняется.
